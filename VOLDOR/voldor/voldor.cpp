...
void VOLDOR::init(vector<Mat> _flows,
	...
	for (int i = 0; i < n_depth_priors; i++)
		depth_prior_confs.push_back(Mat::ones(w, h, CV_32F));

	// init rigidnesses
	for (int i = 0; i < n_flows; i++) 
		rigidnesses.push_back(Mat::ones(w, h, CV_32F));
...
void VOLDOR::save_result(string save_dir) {
	...
	Mat rigidness_sum = Mat::zeros(w, h, CV_32F);
	for (int i = 0; i < n_flows; i++)
...
void VOLDOR::debug() {
	...
	Mat rigidnesses_world = Mat::zeros(w * cfg.viz_img_per_row, h*vis_img_per_col, CV_32F);
