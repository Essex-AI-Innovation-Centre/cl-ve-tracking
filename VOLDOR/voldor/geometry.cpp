...
int optimize_camera_pose(vector<Mat> flows, vector<Mat> rigidnesses,
	...
	if (poses_pool_used == 0) {
		printf("Failure: poses_pool_used == 0\n");
		return 0;
	}
	...
	if (checkRange(pose_opm)) { //check if gpu gives nan...
		...
		// printf("Success: checkRange(pose_opm) passed\n");
		return 1;
	}
	else {
		printf("Failure: checkRange(pose_opm) failed\n");
		return 0;
	}
}
