...
int py_voldor_wrapper(
	...
	if (disparity_pt)
		disparity = Mat(w, h, CV_32F, (void*)disparity_pt);
	if (disparity_pconf_pt)
		disparity_pconf = Mat(w, h, CV_32F, (void*)disparity_pconf_pt);

	for (int i = 0; i < N_dp; i++) {
		depth_priors.push_back(Mat(w, h, CV_32F, (void*)(depth_priors_pt + i * w*h)));
		depth_prior_poses.push_back(Vec6f(depth_prior_poses_pt + i * 6));
		if (depth_prior_pconfs_pt)
			depth_priors_pconfs.push_back(Mat(w, h, CV_32F, (void*)(depth_prior_pconfs_pt + i * w*h)));
	}
	...
	if (depth_conf_pt) {
		Mat depth_conf = Mat::zeros(w, h, CV_32F);
		for (int i = 0; i < voldor.n_flows; i++)
